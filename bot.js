...
// Sulliven's Function (discord has doc's that deny if message.author.bot: rejected by default)
// You can remove this function if it dont work 
function allowBotMessages(client, botIds = []) {
  const allow = new Set(botIds.map(String));
  const original = client.emit;

  const passthrough = (target, prop) => {
    const value = Reflect.get(target, prop);
    return typeof value === 'function' ? value.bind(target) : value;
  };

  const unmask = (message) => {
    const author = new Proxy(message.author, {
      get: (t, p) => (p === 'bot' ? false : passthrough(t, p)),
    });
    return new Proxy(message, {
      get: (t, p) => (p === 'author' ? author : passthrough(t, p)),
    });
  };
  ...
  client.emit = function (event, ...args) {
    if (event === 'messageCreate') {
      ...
      if (message?.author?.bot && id !== client.user?.id && allow.has(String(id))) {
        return original.call(this, event, unmask(message), ...args.slice(1));
      }
    }
    return original.call(this, event, ...args);
  };

  return () => { client.emit = original; };
}

allowBotMessages(client, [
  '1324900026593575044',
]);
